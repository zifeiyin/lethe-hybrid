...
#ifndef lethe_turbulenceModel_solver_h
#define lethe_turbulenceModel_solver_h
...
#include <core/turbulenceModel_solver_strategy.h>
...
public:
   ...
   *
   */
  TurbulenceModelSolver(const Parameters::NonLinearSolver non_linear_solver_parameters);
   ...
   *
   */
  virtual ~TurbulenceModelSolver()
  {
    delete turbulenceModel_solving_strategy;
  }
  ...
  {
    return turbulenceModel_solving_strategy->get_current_newton_iteration();
  }
...
private:
  TurbulenceModelSolverStrategy<VectorType> *turbulenceModel_solving_strategy;
};

template <typename VectorType>
TurbulenceModelSolver<VectorType>::TurbulenceModelSolver(
  ...
  switch (non_linear_solver_parameters.solver)
    {
      case Parameters::NonLinearSolver::SolverType::newton:
        turbulenceModel_solving_strategy =
...
PhysicsSolver<VectorType>::PhysicsSolver()
  : pcout(std::cout, Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
{
  turbulenceModel_solving_strategy = new LinearSolverStrategy<VectorType>(this);
}
...
{
  {
    this->turbulenceModel_solving_strategy->solve();
  }
}
#endif

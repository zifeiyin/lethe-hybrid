...
#ifndef lethe_turbulence_k_omega_h
#define lethe_turbulence_k_omega_h
...
DeclException1(
  TurbulenceModelBoundaryConditionMissing,
  types::boundary_id,
  << "The boundary id: " << arg1
  << " is defined in the triangulation, but not as a boundary condition for the turbulence model. Lethe does not assign a default boundary condition to boundary ids. Every boundary id defined within the triangulation must have a corresponding boundary condition defined in the input file.");
...
template <int dim>
class TurbulenceKOmega : public AuxiliaryTurbulenceModel<dim, GlobalVectorType>
{
public:
  /**
   * @brief Constructor of the TurbulenceKOmega object.
   *
   * @param turbulence_interface Map of the auxiliary turbulence models that will be
   ...
   *
   */
  TurbulenceKOmega(TurbulenceInterface<dim>      *turbulence_interface,
               const SimulationParameters<dim> &p_simulation_parameters,
               std::shared_ptr<parallel::DistributedTriangulationBase<dim>>
                                                  p_triangulation,
               std::shared_ptr<SimulationControl> p_simulation_control)
    : AuxiliaryTurbulenceModel<dim, GlobalVectorType>(
        p_simulation_parameters.turbulence_solving_strategy.at(
          PhysicsID::turbulence_k_omega))
    , turbulence_interface(turbulence_interface)
  ...
  /**
   * @brief Prepare TurbulenceKOmega to write checkpoint.
   */
  void
...
private:
  ...
  TurbulenceInterface<dim> *turbulenceKOmega;
